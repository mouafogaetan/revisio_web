  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center mb-4">
        <Button variant="ghost" onClick={() => navigate(`/lesson/${classeId}/${matiereId}/${chapitreId}/${lessonId}`)} className="mr-4"><ArrowLeft className="w-4 h-4 mr-2" />Retour</Button>
        <div><h2 className="text-xl font-bold text-gray-800">{lesson.lessonName}</h2><p className="text-sm text-gray-500">Cours vidéo</p></div>
      </div>
      ...
      {selectedVideo ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 flex-1 overflow-y-auto">
          {videos.map(video => <div key={video.coursVideoId} onClick={() => setSelectedVideo(video)} className="cursor-pointer bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow overflow-hidden border border-gray-200">
      ...
      <FullScreenAdModal visible={showFullScreenAd} onClose={() => setShowFullScreenAd(false)} durationMs={5000} title="Vidéo" />
    </div>
  )
}